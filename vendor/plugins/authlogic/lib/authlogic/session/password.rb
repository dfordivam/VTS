        ...
        private
          ...
          def validate_by_password
            ...
            if attempted_record.blank?
              generalize_credentials_error_messages? ? add_general_credentials_error : errors.add(login_field, I18n.t('error_messages.login_not_found', :default => "you entered is not correct / incorrect. Please check that the Caps Lock is not turned on and try again."))
              return
            end

            if !attempted_record.send(verify_password_method, send("protected_#{password_field}"))
              self.invalid_password = true
              generalize_credentials_error_messages? ? add_general_credentials_error : errors.add(password_field, I18n.t('error_messages.password_invalid', :default => "you entered is not correct / incorrect. Please check that the Caps Lock is not turned on and try again."))
              return
            end
          end
